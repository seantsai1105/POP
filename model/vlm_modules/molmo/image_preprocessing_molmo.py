...
def resize_and_pad(
    ...
    # scaled_height = int(np.array(height, np.float32) * image_scale_y)
    # scaled_width = int(np.array(width, np.float32) * image_scale_x)

    # -- 8/2 -- We directly resize image without keeping its original ratio
...
class MolmoImageProcessor(BaseImageProcessor):
    ...
    def image_to_patches_and_tokens(
        ...
        base_image_input_d = image_patch_size # 14
        tokens_per_image = image_token_length_w * image_token_length_h # 12 * 12 = 144
        image_base_patch_w = base_image_input_size[1] // base_image_input_d # 24
        image_base_patch_h = base_image_input_size[0] // base_image_input_d # 24
        
        original_image_h, original_image_w = image.shape[:2]
        crop_size = base_image_input_size[0] # 336
        
        # Discard this many patches from the (left/top, right/bottom) of crops
        left_margin, right_margin = overlap_margins # 4, 4
        
        assert left_margin % 2 == 0  # Required for compatibility with 2x2 pooling
        total_margin_pixels = base_image_input_d*(right_margin + left_margin)  # pixels removed per dim, 24 * (4 + 4) = 192
        crop_patches = base_image_input_size[0] // base_image_input_d  # patches per crop dim, 24
        crop_window_patches = crop_patches - (right_margin + left_margin)  # usable patches, 24 - 8 = 16
        ...
        # We assume 2x2 pooling, but can allow padding the right/bottom with extra patches if the number of patches per side is not even
