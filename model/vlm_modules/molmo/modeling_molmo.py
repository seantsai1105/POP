...
from transformers import PreTrainedModel, GenerationConfig, PretrainedConfig
...
@torch.no_grad()
def get_attention_weight(query, key, value, attn_mask=None, dropout_p=0.0,
...
class MolmoBlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        if self.flash_attn_func is not None and attn_mask is None:
            ...
            attn, attn_weight = get_attention_weight(
                q,
                k,
                v,
                attn_mask=attn_mask,
    ...
    def attention(
        ...
        return self.attn_out(att), attn_weight, present
...
class MolmoSequentialBlock(MolmoBlock):
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            att, attn_weight, cache = self.attention(q, k, v, attention_bias, position_ids=position_ids, layer_past=layer_past, use_cache=use_cache)
    ...
    def forward(
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                x, attn_weight, cache = block(x, attention_bias=attention_bias, position_ids=position_ids, layer_past=layer_past, use_cache=use_cache)
            ...
            for group_idx, block_group in enumerate(self.transformer.block_groups):
                ...
                x, attn_weight, cache = block_group(
